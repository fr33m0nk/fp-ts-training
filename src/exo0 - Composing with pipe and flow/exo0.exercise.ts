...
// odd.
type IsOddPiped = (value: number) => boolean
export const isOddP: IsOddPiped = value => pipe(
  value,
  isEven,
  not
);

type IsOddFlow = (value: number) => boolean
export const isOddF: IsOddFlow = flow(
  isEven,
  not
);
...
type Next = (value: number) => number
export const next: Next = value => pipe(
  value,
  isEven,
  ifThenElse(
    () => value / 2,
...
type Next3 = (value: number) => number
export const next3: Next3 = flow(
  next, next, next
);
